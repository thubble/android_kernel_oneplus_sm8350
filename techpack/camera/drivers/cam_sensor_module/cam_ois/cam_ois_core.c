...
int ois_power_down_thread(void *arg)
{
	...
	if (!o_ctrl || !soc_private || !power_info) {
		CAM_ERR(CAM_OIS, "failed: o_ctrl %pK, soc_private %pK, power_info %pK",
				o_ctrl, soc_private, power_info);
		return -EINVAL;
	}
	...
	for (i = 0; i < (OIS_POWER_DOWN_DELAY / 50); i++) {
		...
		if (!IsOISReady(o_ctrl)) {
			CAM_ERR(CAM_OIS, "ois type=%d, is not ready!",
					o_ctrl->ois_type);
			break;
		}
#endif
		/*
		 * sleep 50ms every time and
		 * sleep OIS_POWER_DOWN_DELAY/50 times.
		 */
		msleep(50);
	...
	if ((!o_ctrl->ois_power_down_thread_exit) &&
		...
		if (!rc){
			...
			CAM_ERR(CAM_OIS, "ois type=%d,cam_ois_power_down failed",
					o_ctrl->ois_type);
		}
		...
		CAM_ERR(CAM_OIS, "ois type=%d,No need to do power down, ois_power_down_thread_exit %d, ois_power_state %d",
				o_ctrl->ois_type,
				o_ctrl->ois_power_down_thread_exit,
				o_ctrl->ois_power_state);
	}
...
#ifdef OPLUS_FEATURE_CAMERA_COMMON
		if (o_ctrl->ois_change_cci && o_ctrl->ois_type == 0 &&
				chip_version_old) {
			o_ctrl->io_master_info.cci_client->cci_device =
					CCI_DEVICE_0;
...
static int cam_ois_pkt_parse(struct cam_ois_ctrl_t *o_ctrl, void *arg)
{
		...
		for (i = 0; i < csl_packet->num_cmd_buf; i++) {
			...
			case CAMERA_SENSOR_CMD_TYPE_PWR_DOWN:
				...
				if (o_ctrl->ois_power_state ==
					...
					if (!rc) {
						CAM_ERR(CAM_OIS, "ois type=%d,cam_sensor_update_power_settings successfully",
							o_ctrl->ois_type);
					} else {
		...
		if (o_ctrl->cam_ois_state != CAM_OIS_CONFIG) {
			...
			if (o_ctrl->ois_power_state == CAM_OIS_POWER_OFF) {
				...
				if (!rc){
					o_ctrl->ois_power_state =
						CAM_OIS_POWER_ON;
					CAM_ERR(CAM_OIS, "ois type=%d,cam_ois_power_up successfully",
						o_ctrl->ois_type);
				} else {
					CAM_ERR(CAM_OIS, "ois type=%d,cam_ois_power_up failed",
						o_ctrl->ois_type);
					mutex_unlock(&(
		...
		if (o_ctrl->ois_fw_flag) {
			...
			if (strstr(o_ctrl->ois_name, "lc898")) {
				o_ctrl->ois_module_vendor =
					(o_ctrl->opcode.pheripheral & 0xFF00) >> 8;
				o_ctrl->ois_actuator_vendor =
					o_ctrl->opcode.pheripheral & 0xFF;
				if (o_ctrl->cam_ois_download_fw_in_advance) {
					mutex_lock(&(o_ctrl->do_ioctl_ois));
					if (o_ctrl->ois_download_fw_done ==
						CAM_OIS_FW_NOT_DOWNLOAD) {
						rc = DownloadFW(o_ctrl);
					} else {
						CAM_INFO(CAM_OIS,
							"OIS FW Have Download");
					}
					if (rc)
						o_ctrl->ois_download_fw_done =
							CAM_OIS_FW_NOT_DOWNLOAD;
					else
						o_ctrl->ois_download_fw_done =
	...
	case CAM_OIS_PACKET_OPCODE_OIS_CONTROL:
		...
		if (!IsOISReady(o_ctrl)) {
			...
			for (count = 0; count < o_ctrl->soc_info.num_rgltr; count++) {
				enable = regulator_is_enabled(regulator_get(
					o_ctrl->soc_info.dev,
					o_ctrl->soc_info.rgltr_name[count]));
				CAM_ERR(CAM_OIS, "regulator enable=%d,name[%d]=%s",
					enable, count,
					o_ctrl->soc_info.rgltr_name[count]);
			}
		}
	...
	case CAM_OIS_PACKET_OPCODE_READ: {
		...
		if (o_ctrl->ois_eis_function == 1) {
			rc = OIS_READ_HALL_DATA_TO_UMD(o_ctrl,
					&i2c_read_settings);
			if (rc < 0) {
...
void cam_ois_shutdown(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	if (o_ctrl->cam_ois_state >= CAM_OIS_CONFIG) {
		...
		if (o_ctrl->ois_power_state == CAM_OIS_POWER_ON &&
				o_ctrl->ois_power_down_thread_state ==
				CAM_OIS_POWER_DOWN_THREAD_STOPPED) {
			o_ctrl->ois_power_down_thread_exit = false;
			kthread_run(ois_power_down_thread, o_ctrl,
					"ois_power_down_thread");
			CAM_ERR(CAM_OIS, "ois type=%d,ois_power_down_thread created",
					o_ctrl->ois_type);
		} else {
			CAM_ERR(CAM_OIS, "ois type=%d,no need to create ois_power_down_thread, ois_power_state %d, ois_power_down_thread_state %d",
					o_ctrl->ois_type,
					o_ctrl->ois_power_state,
					o_ctrl->ois_power_down_thread_state);
		}
...
int cam_ois_driver_cmd(struct cam_ois_ctrl_t *o_ctrl, void *arg)
{
	...
	case CAM_ACQUIRE_DEV:
		...
		if (o_ctrl->cam_ois_download_fw_in_advance) {
			...
			if (o_ctrl->ois_power_state == CAM_OIS_POWER_ON) {
				CAM_INFO(CAM_OIS,
					"ois need to exit power down thread");
				o_ctrl->ois_power_down_thread_exit = true;
			}
			mutex_unlock(&(o_ctrl->ois_power_down_mutex));
			mutex_lock(&(o_ctrl->do_ioctl_ois));
	...
	case CAM_CONFIG_DEV:
		...
		if (o_ctrl->cam_ois_download_fw_in_advance) {
			mutex_lock(&(o_ctrl->do_ioctl_ois));
			if (o_ctrl->ois_fd_have_close_state !=
					CAM_OIS_IS_OPEN) {
	...
	case CAM_RELEASE_DEV:
		...
		if (o_ctrl->cam_ois_state == CAM_OIS_CONFIG) {
			o_ctrl->ois_poll_thread_control_cmd =
	...
	case CAM_GET_OIS_EIS_HALL: {
		...
		if (o_ctrl->cam_ois_state == CAM_OIS_START ||
			...
			if (get_hall_version == GET_HALL_DATA_VERSION_V2) {
				ReadOISHALLDataV2(o_ctrl,
						u64_to_user_ptr(cmd->handle));
			} else if (get_hall_version ==
					GET_HALL_DATA_VERSION_V3) {
				ReadOISHALLDataV3(o_ctrl,
						u64_to_user_ptr(cmd->handle));
			} else {
				ReadOISHALLData(o_ctrl,
						u64_to_user_ptr(cmd->handle));
			}
		} else {
			CAM_DBG(CAM_OIS, "OIS in wrong state %d",
					o_ctrl->cam_ois_state);
		}
		break;
	}
